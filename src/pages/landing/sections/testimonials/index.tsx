...
function Testimonials() {
  ...
  return (
    <section id="testimonial" className="section">
      ...
      <div className="section-des">{testimonialData.description}</div>
      <p
        className="content-670"
        dangerouslySetInnerHTML={{
          __html: markdownToHTML(testimonialData.paragraphe),
        }}></p>

      <ul className="skills-history block-right">
        {testimonialData.skillsHistory.map((skill, i: number) => (
          <li key={'skill=item-' + i}>
      ...
      {/* <ul className="skills-progress">
        {testimonialData.skillsProgress.map((prog, i) => (
          <li key={'prog' + i}>
